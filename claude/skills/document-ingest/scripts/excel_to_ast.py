...
class WorkbookSummarizer:
    ...
    def summarize(self):
        wb = openpyxl.load_workbook(self.path, read_only=False, data_only=False)
        ...
        for name in wb.sheetnames:
            ...
            max_row_all = max(max_row_all, rows)
            max_col_all = max(max_col_all, cols)

            sheets_out[name] = {
                "dimensions": dim,
                "state": ws.sheet_state,
                "max_row": rows if rows else None,
                "max_column": cols if cols else None,
    ...
    @staticmethod
    def _decide_route(file_size, visible_sheet_count, max_row, max_col,
                      has_merged, has_formulas):
        reasons = []
        if file_size > 200 * 1024:
            reasons.append(f"file_size>200KB ({file_size} bytes)")
        if visible_sheet_count > 1:
            reasons.append(f"visible_sheet_count={visible_sheet_count}")
        if max_row > 500:
            reasons.append(f"max_row={max_row}")
        if max_col > 20:
            reasons.append(f"max_column={max_col}")
        if has_merged:
            reasons.append("has_merged_cells")
        if has_formulas:
            reasons.append("has_formulas")
        return {
            "target": "ast" if reasons else "markdown",
            "reasons": reasons,
        }
